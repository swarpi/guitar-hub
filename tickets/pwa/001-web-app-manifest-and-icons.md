...
- [ ] `public/manifest.json` exists with the following fields: `name: "Guitar Hub"`, `short_name: "Guitar Hub"`, `start_url: "/"`, `display: "standalone"`, `background_color: "#faf9f3"` (matches the `--color-page` canvas value), `theme_color: "#1f3a2e"` (matches the header forest green), and an `icons` array referencing the 192x192 and 512x512 PNGs
- [ ] `public/icons/icon-192x192.png` exists — a simple, recognizable icon at 192x192 px using the forest green / ivory palette
- [ ] `public/icons/icon-512x512.png` exists — same icon at 512x512 px (used for the splash screen and high-DPI displays)
- [ ] `src/app/layout.tsx` includes `<link rel="manifest" href="/manifest.json" />` in the rendered `<head>` — added via Next.js `metadata.manifest` export or direct `<link>` tag in the layout
- [ ] `src/app/layout.tsx` includes `<meta name="theme-color" content="#1f3a2e" />` so the browser's address bar and system UI adopt the app's color
- [ ] `src/app/layout.tsx` includes `<link rel="apple-touch-icon" href="/icons/icon-192x192.png" />` for iOS home screen (Safari does not read manifest icons)
- [ ] In Chrome on Android, visiting the deployed app triggers or makes available the "Add to Home Screen" install option
- [ ] When launched from the home screen, the app renders without the browser URL bar (standalone mode)
- [ ] The installed app's title bar / status bar uses `#1f3a2e` as the theme color
- [ ] `pnpm build` passes
- [ ] `pnpm lint` passes
